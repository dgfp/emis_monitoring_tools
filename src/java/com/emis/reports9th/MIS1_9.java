 ...
 */
@WebServlet(name = "MIS1_9", urlPatterns = {"/mis-form-1"})
public class MIS1_9 extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Menu.setMenu("VIEW REPORTS", "mis-form-1", request);
        String view = "WEB-INF/jsp/reports9th/mis-1-9.jsp";
        try {
            if (request.getSession(false).getAttribute("district") != null) {
                if (new UserDao().isPaperless(request.getSession(false).getAttribute("district").toString())) {
                    response.sendRedirect(request.getContextPath() + "/mis-form-1-test");
                } else {
...
//        request.setAttribute("userLevel", (String) request.getSession(false).getAttribute("userLevel"));
//        Menu.setMenu("VIEW REPORTS", "mis-form-1", request);
//        request.getRequestDispatcher("WEB-INF/jsp/reports9th/mis-1-9.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equalsIgnoreCase("showdata")) {
            try {
                ...
                if (isSubmitable) {
                    ...
                    response.setContentType("text/plain;charset=UTF-8");
                    response.getWriter().write("{\"MIS\":" + mis1.get(0) + "," + mis1.get(1) + ",\"LMIS\":" + LMIS.get(0) + ",\"modrep\":" + modeRep + ",\"isSubmittable\":" + isSubmitable + ",\"submissionId\":" + submissionId + ",\"status\":" + isApprove + ",\"submissionDate\":\"" + submissionDate.split(" ")[0] + "\"}");
        ...
        if (action.equalsIgnoreCase("submitReport")) {
            ...
            try {
                ...
                lmisJsonObj.put("unionid", unionId);
                lmisJsonObj.put("unit", fwaUnit);
                lmisJsonObj.put("providerid", provCode);
                lmisJsonObj.put("supervisorid", supervisorCode + "");
                ...
                String query = "INSERT INTO rpt_mis_form1_lmis_9v (%s) VALUES (%s) ON CONFLICT(%s) DO UPDATE SET (%s)=row(%s);";
    ...
    private ArrayList<String> getMis1RealData(String divisionId, String districtId, String upazilaId, String unionId, String fwaUnit, String month, String year, String date, String[] dates, String provCode, HttpServletRequest request) {
        ...
        try {
            ...
            if (reportType.equals("monthWise")) {
                sql_ka = "SELECT * from fn_mis_form1_ka_new_9v(" + districtId + "," + upazilaId + "," + unionId + ",cast(null as integer)," + fwaUnit + "," + year + "," + month + "," + provCode + ")";
                sql_kha_ga_gha_uma_cha = "SELECT * from fn_mis_form1_kha_ga_gha_uma_cha_9v (" + districtId + "," + upazilaId + "," + unionId + ",cast(null as integer),'" + fwaUnit + "'," + year + "," + month + "," + provCode + ");";

            } else if (reportType.equals("dateWise")) {
                startDate = Date.convertToISO(startDate);
                endDate = Date.convertToISO(endDate);
                sql_ka = "select * from public.fn_mis_form1_ka_new_9v_date_range(" + districtId + "," + upazilaId + "," + unionId + ",cast(null as integer)," + fwaUnit + "," + year + "," + month + ",'" + startDate + "','" + endDate + "'," + provCode + ")";
                sql_kha_ga_gha_uma_cha = "select * from public.fn_mis_form1_kha_ga_gha_uma_cha_9v_date_range (" + districtId + "," + upazilaId + "," + unionId + ",cast(null as integer)," + fwaUnit + "," + year + "," + month + ",'" + startDate + "','" + endDate + "'," + provCode + ")";
            }
            //String query = "SELECT * from fn_mis_form1_ka_new_9v(" + districtId + "," + upazilaId + "," + unionId + ",cast(null as integer)," + fwaUnit + "," + year + "," + month + "," + provCode + ")";
            //String query2 = "SELECT * from fn_mis_form1_kha_ga_gha_uma_cha_9v (" + districtId + "," + upazilaId + "," + unionId + ",cast(null as integer),'" + fwaUnit + "'," + year + "," + month + "," + provCode + ");";
            //ResultSet result = new DBManagerDistrict(Integer.parseInt(districtId)).select(sql_ka);
            //ResultSet result2 = new DBManagerDistrict(Integer.parseInt(districtId)).select(sql_kha_ga_gha_uma_cha);
    ...
    private JSONArray getLMIS(String districtId, String upazilaId, String unionId, String fwaUnit, String month, String year, String provCode) {
        ...
        try {
            String query = "SELECT * FROM public.rpt_mis_form1_lmis_9v where month_no =" + month + " and year_no=" + year + " and zillaid=" + districtId + " and upazilaid=" + upazilaId + " and unionid=" + unionId + " and unit=" + fwaUnit + " and providerid=" + provCode;
