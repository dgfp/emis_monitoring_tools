...
import com.emis.service.LmisDataEntryService;
 ...
 */
@WebServlet(name = "MIS1_9_lmis_entry", urlPatterns = {"/mis-form-1-test-lmis-entry"})
public class MIS1_9_test_lmis_entry extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String view = "WEB-INF/jsp/reports9th/mis-1-9-test-lmis-entry.jsp";
        try {
...
//        String[] dates = date.split("~"); //dates[0];
        ...
        try {
            ReportSubmissionService service = new ReportSubmissionService(Integer.parseInt(districtId));
            LmisDataEntryService lmisDataEntryService = new LmisDataEntryService(Integer.parseInt(districtId));
            UnitMaster um = service.getUnitId(Integer.parseInt(districtId), Integer.parseInt(upazilaId), Integer.parseInt(unionId), Integer.parseInt(fwaUnit));
            String unitId = String.valueOf(um.getUnitid());
            
            if (action.equalsIgnoreCase("showdata")) {
                String data = lmisDataEntryService.getLmisData(districtId, upazilaId, unionId, fwaUnit
                        , unitId, provCode, month, year);
                ...
                lmisJsonObj.put("reporting_upazilaid", upazilaId);
                ...
                lmisJsonObj.put("entry_date", "now()");
                lmisJsonObj.put("systementrydate", "now()");
                lmisJsonObj.put("modifydate", "now()");
                lmisJsonObj.put("created_by", provCode);
                int resultLmisEntry = lmisDataEntryService.insertOnUpdateData(districtId, lmisJsonObj);
                String data = lmisDataEntryService.getLmisData(districtId, upazilaId, unionId, fwaUnit
                        , unitId, provCode, month, year);
                String r = "{\"rowCount\":"+resultLmisEntry + ", " + "\"dataLmis\":" + data +"}";
                response.setContentType("text/plain;charset=UTF-8");
                ...
                String r = "{\"rowCount\":"+ 0 + ", " + "\"dataLmis\":" + data +"}";
                response.setContentType("text/plain;charset=UTF-8");
                response.getWriter().write(r);
            } 
        } catch (Exception e) {
...
//                response.getWriter().write("{\"serverResponse\": \"warning\",\"message\": \""+unitId+"\"}");
        // LocalDate.now().getYear()
//                if (new MISService(Integer.parseInt(districtId)).checkAdditionalUnitByProvider(Integer.parseInt(provCode), Integer.parseInt(fwaUnit)) == 2 && !new MISDao(this.toInt(districtId)).isSubmittedAsAdditional(districtId, upazilaId, unionId, fwaUnit, month, year)) {
...
//                        response.getWriter().write("{\"serverResponse\": \"success\",\"MIS\":" + mis1.get(0) + "," + mis1.get(1) + ",\"LMIS\":{},\"modrep\":" + modeRep + ",\"isSubmittable\":" + isSubmitable + ",\"submissionId\":" + submissionId + ",\"assignType\":" + mis1.get(2) + ",\"unionids\":\"" + mis1.get(3) + "\",\"unitid\":" + mis1.get(4) + ",\"bbsunionid\":" + mis1.get(5) + ",\"reporting_upazilaid\":" + mis1.get(6) + "}");
//                        //response.setContentType("text/plain;charset=UTF-8");
...
//                        response.getWriter().write("{\"serverResponse\": \"success\",\"MIS\":" + mis1.get(0) + "," + mis1.get(1) + ",\"LMIS\":" + LMIS.get(0) + ",\"modrep\":" + modeRep + ",\"isSubmittable\":" + isSubmitable + ",\"submissionId\":" + submissionId + ",\"status\":" + isApprove + ",\"assignType\":" + mis1.get(2) + ",\"unionids\":\"" + mis1.get(3) + "\",\"unitid\":" + mis1.get(4) + ",\"bbsunionid\":" + mis1.get(5) + ",\"reporting_upazilaid\":" + mis1.get(6) + ",\"submissionDate\":\"" + submissionDate.split(" ")[0] + "\"}");
//                        //response.setContentType("text/plain;charset=UTF-8");
...
//
//            String reporting_upazilaid = request.getParameter("reporting_upazilaid"), bbsunionid = request.getParameter("bbsunionid"), unionids = request.getParameter("unionids"), unitid = request.getParameter("unitid");
//
            try {
//                //get supervisor code
...
//
    ...
    private void getLMISData(String districtId, String upazilaId, String unionId, String fwaUnit, int unitId, String month, String year, String provCode) {
    ...
    private ArrayList<String> getMis1RealData(String divisionId, String districtId, String upazilaId, String unionId, String fwaUnit, String month, String year, String date, String[] dates, String provCode, HttpServletRequest request) {
        ...
        try {
            ...
            System.out.println("Array of union: " + unionids);
            System.out.println("Unique Unit ID: " + unitid);
            System.out.println("BBS union ID: " + bbsunionid);
