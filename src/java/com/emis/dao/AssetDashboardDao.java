...
public class AssetDashboardDao {
    ...
    public AssetDashboardDao() {
        db = new DBManagerMonitoring();
        connection = db.openConnection();
    }

    public AssetDashboardDao(int districtId) {
        db = new DBManagerDistrict(districtId);
        connection = db.openConnection();
    }

    public JSONArray getProcurementAndProjection(AssetReport asset) throws SQLException, Exception {
        Print.o("\n\n\nTablet Stock Status:-");
        String sql = "select \n"
                + "	modelname, \n"
        ...
        sql = "select \n"
                ...
                + "	,(datediff('year', purchaseddate, now()::date)) age_of_tablets\n"
                ...
                + "	,(purchaseddate + interval '3 year')::date as end_of_funcional_life\n"
                + "	,count(*) filter(where (purchaseddate + interval '3 year')::date<now()::date) as procurement_required\n"
                + "	,(purchaseddate + interval '2 year')::date as begin_procurement_process\n"
                + "from web_asset_device_master ms\n"
                + "left join web_asset_distribution_history dh using (imei1)\n"
                + "left join web_asset_model ml on ms.model::int = ml.modelid \n"
                + "where dh.active = 1  \n"
                + "group by purchaseddate\n"
                + "order by purchaseddate, age_of_tablets";
        Print.o(sql);
    ...
    public JSONArray getAgeOfTablet(AssetReport asset) throws SQLException, Exception {
        ...
        sql = " select \n"
                ...
                + "	,count(*) tablet_purchased\n"
                + "	,count(*) filter(where (datediff('month', dh.receiveddate::date, now()::date)) < 12 ) as less_than_12_months\n"
                + "	,count(*) filter(where (datediff('month', dh.receiveddate::date, now()::date)) > 12 ) as over_12_months\n"
                + "	,count(*) filter(where (datediff('month', dh.receiveddate::date, now()::date)) > 24 ) as over_24_months\n"
                + "	,count(*) filter(where (datediff('month', dh.receiveddate::date, now()::date)) > 36 ) as over_36_months\n"
                + "from web_asset_device_master ms\n"
                + "left join web_asset_distribution_history dh using (imei1)\n"
                + "left join web_asset_model ml on ms.model::int = ml.modelid \n"
    ...
    public JSONArray getTotalAsset(AssetReport asset) throws SQLException, Exception {
        Print.o("\n\n\nTotal Asset:-");
        String sql = "SELECT count(*) total_asset FROM public.web_asset_device_master where active=1 " + this.getWhereByUserLevel(asset);
        Print.o(sql);
        ResultSet resultSet = connection.createStatement().executeQuery(sql);
        connection.close();
        return Convertor.toJSON(resultSet);
