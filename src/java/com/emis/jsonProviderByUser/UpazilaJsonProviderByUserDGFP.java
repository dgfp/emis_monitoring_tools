 ...
 * @author Helal
 */
@WebServlet(name = "UpazilaJsonProviderByUserDGFP", urlPatterns = {"/UpazilaJsonProviderByUserDGFP"})
public class UpazilaJsonProviderByUserDGFP extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String sql = "select zillaid, upazilaid, reporting_upazilaname upazilaname, reporting_upazilanameeng upazilanameeng from reporting_upazila "+where;
...
//                "where p.supervisorcode in (select distinct p2.providerid from providerdb p2 where p2.supervisorcode = "+request.getSession(false).getAttribute("providerCode")+")";
                sql = "select ru.zillaid, ru.upazilaid, ru.reporting_upazilaname upazilaname, ru.reporting_upazilanameeng upazilanameeng  \n" +
                "from reporting_upazila_ufpo ruu join reporting_upazila ru using (zillaid, upazilaid) where providerid = "+request.getSession(false).getAttribute("providerCode");
