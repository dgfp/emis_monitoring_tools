 ...
 * @author Helal
 */
@WebServlet(name = "ProviderManagementDGFP", urlPatterns = {"/provider-management-dgfp"})
public class ProviderManagementDGFP extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        Menu.setMenu("MANAGEMENT", "provider-management-dgfp", request);
        request.getRequestDispatcher("WEB-INF/jsp/management/ProviderManagementDGFP.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Insert devicesetting on history table
        HttpSession session = request.getSession(false);
//            ProviderManagement pm = new ProviderManagement(Integer.parseInt(globalDistrictId));
        String mToolsUserName = session.getAttribute("username").toString();
        ...
        if (action.equalsIgnoreCase("showdata")) {
            ...
            if (upazilaId.length() == 0 && unionId.length() == 0) {
                if (providerType.length() == 0) {
                    condition = "WHERE pdb.zillaid = " + districtId;
                } else {
                    condition = "WHERE pdb.zillaid = " + districtId + " AND pt.provtype=" + providerType;
                }
            } else if (unionId.length() == 0) {
                if (providerType.length() == 0) {
                    condition = "WHERE pdb.zillaid=" + districtId + " AND  pdb.upazilaid=" + upazilaId;
                } else {
                    condition = "WHERE pdb.zillaid=" + districtId + " AND  pdb.upazilaid=" + upazilaId + " AND pt.provtype=" + providerType;
                }

            } else if (providerType.length() == 0) {
                condition = "WHERE pdb.zillaid=" + districtId + " AND  pdb.upazilaid=" + upazilaId + " AND pdb.unionid=" + unionId;
            } else {
                condition = "WHERE pdb.zillaid=" + districtId + " AND  pdb.upazilaid=" + upazilaId + " AND pdb.unionid=" + unionId + " AND pt.provtype=" + providerType;
            }
//            String query = "select pdb.active, pdb.supervisorcode, pdb.mobileno, pdb.providerid, pdb.areaupdate, pdb.provtype, pdb.provname, pdb.provnamebang, pdb.email, pdb.divid, pdb.zillaid, pdb.upazilaid, pdb.unionid, pt.typename, pdb.devicesetting, pdb.provpass \n"
//                    + "from providerdb pdb join providertype pt on pt.provtype=pdb.provtype "
//                    + condition + " order by provname asc";
            String query = "select pdb.active, pdb.supervisorcode, pdb.mobileno, pdb.providerid, pdb.areaupdate, pdb.provtype, pdb.provname, "
                    + "pdb.provnamebang, pdb.email, pdb.divid, pdb.zillaid, pdb.upazilaid, pdb.unionid, pt.typename, "
                    + "pdb.devicesetting, pdb.provpass, org.organization_abbr_name_eng, prt.typename_abbr \n"
                    + "from providerdb pdb join providertype pt on pt.provtype=pdb.provtype "
                    + "join organizaions org on pdb.organization_id = org.organization_id "
                    + "join provider_role_type prt on pdb.organization_id = prt.organization_id and pdb.provtype = prt.prov_role_id "
                    + condition + " order by provname asc";
            ...
            String query = "select supervisorcode, mobileno, providerid, providerdb.areaupdate, provname, provnamebang, providertype.provtype, providertype.typename, devicesetting, provpass \n"
            ...
            try {
                new ProviderManagement(Integer.parseInt(globalDistrictId)).insertDeviceSettingHistory(mToolsUserName, providerId, Integer.valueOf(updatedSetting), 1);
            } catch (Exception e) {
            ...
            if (result > 0) {
                try {
                    ProviderManagement p = new ProviderManagement(Integer.parseInt(zillaid));
                    p.setAreaUpdate(Integer.parseInt(providerid), 1);
                } catch (Exception e) {
                    System.out.println(e.getMessage());
                }
            }
            ...
            try {
                ProviderManagement p = new ProviderManagement(districtId);
                p.setAreaUpdate(providerId, 1);
            } catch (Exception e) {
                System.out.println(e.getMessage());
            }
            response.setContentType("text/plain;charset=UTF-8");
            ...
            String provnamebang = request.getParameter("provnamebang");
            ...
            String query = "update public.providerdb \n"
                    + "set zillaid = " + districtId + ", active = " + active + ", upazilaid = " + upazilaId + ",unionid = " + unionId + ",provtype = " + typeId + ",provname ='" + name + "', provnamebang ='" + provnamebang + "', mobileno = '" + mobile + "', email = '" + email + "', supervisorcode = " + supervisor + " ," + modifyDate + " \n"
            ...
            try {
                ...
                int organizationId = request.getParameter("organization_id") != null ? Integer.parseInt(request.getParameter("organization_id")) : 0;
                //mToolsUserName
                String superviserId = ""; //= request.getParameter("superviserCode").length() == 0 ? "0" : request.getParameter("superviserCode");
                if (request.getParameter("superviserCode") == null) {
                    superviserId = "0";
                } else {
                    superviserId = request.getParameter("superviserCode").length() == 0 ? "0" : request.getParameter("superviserCode");
                }
...
//                    }
                    ...
                    query = "insert into providerdb (divid, zillaid, upazilaid, unionid, provtype, providerid, provname, provnamebang, mobileno, email, endate, devicesetting, active, healthidrequest, tablestructurerequest, areaupdate , provpass , supervisorcode, systemupdatedt,organization_id,created_by)"
                            + " values (" + divisonId + ", " + districtId + ", " + upazilaId + ", " + unionId + ", " + provType + ", " + id + ",'" + name + "','" + provnamebang + "', " + phone + ", '" 
                            + email + "', to_date('" + joinDate + "', 'dd/mm/yyyy')  , '2', '1', '2', '2', '2', '" + password + "', " + superviserId + ",'" + systemupdatedt + "'," + organizationId + ",'" + mToolsUserName + "')";
                    System.out.println("SQL: " + query);
                    db = new DBManagerDistrict(districtId);
                    rowCount = db.update(query);

                    //create LoginUser object
                    LoginUserDGFP lu = new LoginUserDGFP();
                    if (provType == 10) {
                        lu.setUSERID(id + "");
                        lu.setUNAME(name);
                        lu.setPASS(password);
                        lu.setUserLevel("5");
                        lu.setDivision(divisonId + "");
                        lu.setDistrict(districtId + "");
                        lu.setUpazila(upazilaId + "");
                        lu.setUnion(unionId + "");
                        lu.setName(name);
                        lu.setEmail(email);
                        lu.setDesignation("7");
                        lu.setProvtype(10);
                        lu.setDesignation_provtype(7);
                        lu.setRole("20");
                        boolean isCompleted = new UserDao().addUserToWebAccessControl(lu);
                        System.out.println("User added to loginuser: " + isCompleted);
            ...
            //------------------------Add area to provider -------------------------------------------------------------------------------------
        } else if (action.equalsIgnoreCase("getSupervisorList")) {
            try {
                int zillaid = this.toInt(request.getParameter("zillaid").toString());
                int upazilaid = this.toInt(request.getParameter("upazilaid").toString());
                int unionid = this.toInt(request.getParameter("unionid").toString());
                int provtype = this.toInt(request.getParameter("provtype").toString());
                response.setContentType("text/plain;charset=UTF-8");
                response.getWriter().write(new ProviderManagement(zillaid).getSupervisor(zillaid, upazilaid, unionid, provtype).toString());
            } catch (Exception ex) {
                ex.printStackTrace();
                response.getWriter().write(new Utility().errorResponse());
            }
        } else if (action.equalsIgnoreCase("getNewProviderId")) {
            try {
                int zillaid = this.toInt(request.getParameter("zillaid").toString());
                String provtype = request.getParameter("provtype").toString();
                response.setContentType("text/plain;charset=UTF-8");
                response.getWriter().write(new ProviderManagement(zillaid).getNewProviderId(zillaid, provtype).toString());
            } catch (Exception e) {
                e.printStackTrace();
                response.getWriter().write(new Utility().errorResponse());
            }
        } else if (action.equalsIgnoreCase("getOrganizations")) {
            try {
                int zillaid = this.toInt(request.getParameter("zillaid").toString());
                int upazilaid = this.toInt(request.getParameter("upazilaid").toString());
                int unionid = this.toInt(request.getParameter("unionid").toString());
                response.setContentType("text/plain;charset=UTF-8");
                response.getWriter().write(new ProviderManagement(zillaid).getOrganizations(zillaid, upazilaid, unionid).toString());
            } catch (Exception e) {
                e.printStackTrace();
                response.getWriter().write(new Utility().errorResponse());
            }
        } else if (action.equalsIgnoreCase("getDesignationByOrganization")) {
            try {
                int zillaid = this.toInt(request.getParameter("zillaid").toString());
                int organizationId = this.toInt(request.getParameter("organizationId").toString());
                int providerType = this.toInt(request.getParameter("providerType").toString());
                response.setContentType("text/plain;charset=UTF-8");
                response.getWriter().write(new ProviderManagement(zillaid).getDesignationByOrganization(zillaid, organizationId, providerType).toString());
            } catch (Exception e) {
                e.printStackTrace();
                response.getWriter().write(new Utility().errorResponse());
            }
        } else if (action.equalsIgnoreCase("getUnitDetailsByFWA")) {
            try {
                int zillaid = this.toInt(request.getParameter("zillaid").toString());
                int providerid = this.toInt(request.getParameter("providerid").toString());
                response.setContentType("text/plain;charset=UTF-8");
                response.getWriter().write(new ProviderManagement(zillaid).getUnitDetailsByFWA(providerid).toString());
            } catch (Exception ex) {
                ex.printStackTrace();
                response.getWriter().write(new Utility().errorResponse());
            }
        } else if (action.equalsIgnoreCase("unassignFwaUnit")) {
            try {
                ProviderUnassign pu = new ObjectMapper().readValue(request.getParameter("data").toString(), ProviderUnassign.class);
                Print.o(pu);
                String msg = "";
                boolean success = false;
                if (new ProviderManagement(pu.getZillaid()).unassignProvider(pu, request)) {
                    msg = "Provider catchment area unassigned successfully";
                    success = true;
                } else {
                    msg = "Provider catchment area unassigned failed";
                    success = true;
                }
            ...
            try {
                ...
                String sql = "update providerdb set active=" + userActive + " where providerid=" + providerId;
                Print.o(sql);
                int rowCount = new DBManagerDistrict(this.toInt(globalDistrictId)).update(sql);

                String loginUser = "update loginuser set active=" + userActive + " where userid='" + providerId + "' and designation in (4,7) and provtype in (10,15);";
                new DBManagerMonitoring().update(loginUser);
