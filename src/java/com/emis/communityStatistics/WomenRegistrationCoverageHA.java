 ...
 */
@WebServlet(name = "WomenRegistrationCoverageHA", urlPatterns = {"/immunization_woman_ha"})
public class WomenRegistrationCoverageHA extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
            request.setAttribute("submenu", "immunization_woman_ha");
            request.setAttribute("menu", "COMMUNITY STATISTICS");
            RequestDispatcher view = request.getRequestDispatcher("WEB-INF/jsp/communityStatistics/womenRegistrationCoverageHA.jsp");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equals("AggregateUpazila")) {
            ...
            String query = "SELECT  * from public.fn_women_aggregate_ward("+districtId+","+upazilaId+","+unionId+",'"+startDate+"','"+endDate+"');";
            try {
                System.out.println(query);
                DBManagerDistrict db = new DBManagerDistrict(Integer.parseInt(districtId));
                ResultSet result = db.select(query);
            ...
            String query = "SELECT  * from public.fn_women_aggregate_village_basedOn_ward("+districtId+","+upazilaId+","+unionId+","+wardId+",'"+startDate+"','"+endDate+"');";
            //SELECT  * from public.fn_women_aggregate_village_basedOn_ward(93,28,69,'1','2014-01-01','2017-12-30');
            try {
                System.out.println(query);
                DBManagerDistrict db = new DBManagerDistrict(Integer.parseInt(districtId));
                ResultSet result = db.select(query);
...
//                }
            ...
            String query = "SELECT  * from public.fn_women_individual_ha_basedOn_ward('"+districtId+"','"+upazilaId+"','"+unionId+"','"+wardId+"','"+villageId+"','"+startDate+"','"+endDate+"');";
            try {
                System.out.println(query);
                DBManagerDistrict db = new DBManagerDistrict(Integer.parseInt(districtId));
                ResultSet result = db.select(query);
