 ...
 * @author Helal
 */
@WebServlet(name = "Pregnant", urlPatterns = {"/pregnant"})
public class Pregnant extends HttpServlet {
...
//        String jsp="WEB-INF/jsp/communityStatistics/pregnantUpdated.jsp";
//        
...
//            jsp="WEB-INF/jsp/communityStatistics/pregnantHA.jsp";
//        }
...
//        
        ...
        Menu.setMenu("COMMUNITY STATISTICS", "pregnant", request);
        request.setAttribute("userLevel", (String) request.getSession(false).getAttribute("userLevel"));
        String view = "WEB-INF/jsp/communityStatistics/pregnantUpdated.jsp";
        try {
            if (request.getSession(false).getAttribute("district") != null) {
                if (new UserDao().isPaperless(request.getSession(false).getAttribute("district").toString())) {
                    response.sendRedirect(request.getContextPath() + "/pregnant-woman-dgfp");
                } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            ...
            if (action.equals("AggregateUpazila")) {
                ...
                String query = "SELECT  * from public.fn_pregwomen_aggregate_unit("+districtId+","+upazilaId+","+unionId+",'"+startDate+"','"+endDate+"');";
                try {
                    System.out.println(query);
                    DBManagerDistrict db = new DBManagerDistrict(Integer.parseInt(districtId));
                    ResultSet result = db.select(query);
                ...
                String query = "SELECT  * from public.fn_pregwomen_aggregate_village("+districtId+","+upazilaId+","+unionId+","+unitId+",'"+startDate+"','"+endDate+"');";
                try {
                    System.out.println(query);
                    DBManagerDistrict db = new DBManagerDistrict(Integer.parseInt(districtId));
                    ResultSet result = db.select(query);
                ...
                String unitId = request.getParameter("unitId").length() == 0 ? null : request.getParameter("unitId");
