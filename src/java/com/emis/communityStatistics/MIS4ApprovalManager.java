 ...
 */
@WebServlet(name = "MIS4ApprovalManager", urlPatterns = {"/mis4-approval-manager"})
public class MIS4ApprovalManager extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        Menu.setMenu("COMMUNITY STATISTICS", "mis4-approval-manager", request);
        String view = "WEB-INF/jsp/communityStatistics/MIS4ApprovalManager.jsp";
        try {
            if (request.getSession(false).getAttribute("district") != null) {
                if (new UserDao().isPaperless(request.getSession(false).getAttribute("district").toString())) {
                    response.sendRedirect(request.getContextPath() + "/mis4-approval-manager-dgfp");
                } else {
...
//        Menu.setMenu("COMMUNITY STATISTICS", "mis4-approval-manager", request);
//        request.getRequestDispatcher("WEB-INF/jsp/communityStatistics/MIS4ApprovalManager.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (action.equalsIgnoreCase("showData")) {
                ...
                String sql = "select * from web_report_submission_list_mis4(" + district + ",15," + reportType + "," + month + "," + year + ");";
            ...
            if (action.equalsIgnoreCase("actionOnResponse")) {
                ...
                if (resultUpdatereportSubmission == 1 && resultInsertReportReview == 1) {
                    ...
                    if (status.equals("1")) {
                        String sql = "select * from public.fn_mis_form4_ufpo_submit_9v (" + district + ", " + upazila + ", " + year + ", " + month + ", " + provCode + ");";
