 ...
 */
@WebServlet(name = "MIS2ApprovalManagerV9DGFP", urlPatterns = {"/MIS2ApprovalProcessDGFP"})
public class MIS2ApprovalManagerV9DGFP extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        request.setAttribute("userLevel", (String) request.getSession(false).getAttribute("userLevel"));
        Menu.setMenu("COMMUNITY STATISTICS", "MIS2ApprovalProcessDGFP", request);
        String view = "WEB-INF/jsp/communityStatistics/MIS2ApprovalManagerV9DGFP.jsp";
        try {
            if (request.getSession(false).getAttribute("district") != null) {
                if (!new UserDao().isPaperless(request.getSession(false).getAttribute("district").toString())) {
                    response.sendRedirect(request.getContextPath() + "/MIS2ApprovalProcess");
                } else {
...
//        request.setAttribute("submenu", "MIS2ApprovalProcessDGFP");
...
//        
//        RequestDispatcher view = request.getRequestDispatcher("WEB-INF/jsp/communityStatistics/MIS2ApprovalManagerV9DGFP.jsp");
...
//        int month = 3, year = 2018, reportType=702;
        ...
        try {
            if (action.equalsIgnoreCase("showData")) {
                ...
                String sql = "select * from web_report_submission_list_mis2_dgfp(p_supervisorcode=>" + provCode + ",p_month=>" + month + ",p_year=>" + year + ");";
            ...
            if (action.equalsIgnoreCase("reviewData")) {
                ...
                String sql2 = "SELECT review_id, submission_id, submission_from, submission_to, status, notes, systementrydate, html  FROM public.web_report_review_dgfp where submission_id=" + submissionId + " order by systementrydate";
            ...
            if (action.equalsIgnoreCase("actionOnResponse")) {
                ...
                String reporting_unionid = request.getParameter("reporting_unionid");
                db.start();
                ...
                if (resultUpdatereportSubmission == 1 && resultInsertReportReview == 1) {
                    ...
                    if (status.equals("1")) {
                        String sql = "INSERT INTO public.web_mis_submission_dgfp(\n"
                                + "zillaid, upazilaid, unionid, reporting_unionid, providerid, month, year, mistype, emis, csba, lmis)\n"
                                + "VALUES (" + district + ", " + upazila + ", " + union + ", " + reporting_unionid + "," + submissionTo + ", " + month + ", " + year + ", 2, 1, 1, 1);";

                        String updateMIS2 = "UPDATE public.rpt_mis_form1_fwa_9v_dgfp SET is_approved=1, modifydate=now() WHERE zillaid = " + district
                                + " and upazilaid = " + upazila + " and reporting_unionid=" + reporting_unionid + " and supervisorcode = "
                                + submissionTo + " and  year_no = " + year + " and month_no = " + month;

                        String updateLMIS = "UPDATE public.rpt_mis_form1_lmis_9v_dgfp SET is_approved=1, modifydate=now() WHERE zillaid = "
                                + district + " and upazilaid = " + upazila + " and reporting_unionid=" + reporting_unionid
                                + " and supervisorcode = " + submissionTo + " and  year_no = " + year + " and month_no = " + month;
