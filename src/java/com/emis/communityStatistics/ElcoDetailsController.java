 ...
 * @author Helal
 */
@WebServlet(name = "ElcoDetailsController", urlPatterns = {"/elco_details"})
public class ElcoDetailsController extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        Menu.setMenu("COMMUNITY STATISTICS", "elco_details", request);
        request.setAttribute("userLevel", (String) request.getSession(false).getAttribute("userLevel"));
        String view = "WEB-INF/jsp/communityStatistics/elco_details.jsp";
        try {
            if (request.getSession(false).getAttribute("district") != null) {
                if (new UserDao().isPaperless(request.getSession(false).getAttribute("district").toString())) {
                    response.sendRedirect(request.getContextPath() + "/eligible-couple");
                } else {
...
//        request.setAttribute("userLevel", (String) request.getSession(false).getAttribute("userLevel"));
//        RequestDispatcher view = request.getRequestDispatcher("WEB-INF/jsp/communityStatistics/elco_details.jsp");
//        view.forward(request, response);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equals("Aggregate")) {
            ...
            String queryFunc1 = "SELECT  * from public.web_elco_aggregate_new(" + districtId + "," + upazilaId + "," + unionId + "," + unitId + ",'" + startDate + "','" + endDate + "');";

            //String queryFunc1="SELECT  * from web_elco_aggregate_update(69,9,19,null,'01/01/2015','17/09/2018')";
            try {

                DBManagerDistrict db = new DBManagerDistrict(Integer.parseInt(districtId));
                System.out.println(queryFunc);
                ResultSet result = db.select(queryFunc);
