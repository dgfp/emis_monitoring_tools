 ...
 * @author RHIS082
 */
@WebServlet(name = "DeathList", urlPatterns = {"/DeathList"})
public class DeathList extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        request.setAttribute("submenu", "DeathList");
        request.setAttribute("menu", "VIEW AS REGISTER");
        RequestDispatcher view = request.getRequestDispatcher("WEB-INF/jsp/viewAsRegister/DeathList.jsp");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String sql = "select * from web_death_list(?,?,?,?,?,?,?)";
            System.out.println("SQL: select * from web_death_list(" + divisionId + "," + districtId + ",'" + upazilaId + "','" + unionId + "','" + unitId + "','" + startDate + "','" + endDate + "')");
