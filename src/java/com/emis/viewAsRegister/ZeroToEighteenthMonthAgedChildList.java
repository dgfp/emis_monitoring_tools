 ...
 * @author RHIS082
 */
@WebServlet(name = "ZeroToEighteenthMonthAgedChildList", urlPatterns = {"/ZeroToEighteenthMonthAgedChildList"})
public class ZeroToEighteenthMonthAgedChildList extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        request.setAttribute("submenu", "ZeroToEighteenthMonthAgedChildList");
        request.setAttribute("menu", "VIEW AS REGISTER");
        RequestDispatcher view = request.getRequestDispatcher("WEB-INF/jsp/viewAsRegister/ZeroToEighteenthMonthAgedChildList.jsp");
...
//        String startDate = request.getParameter("startDate");
        ...
        if (districtId != "" || upazilaId != "" || unionId != "" || unitId != "") {
            try {
                ...
                String sql = "select * from web_zero_to_eighteenth_month_aged_child_list(?,?,?,?,?,?)";
                System.out.println("SQL: select * from web_zero_to_eighteenth_month_aged_child_list(" + divisionId + "," + districtId + ",'" + upazilaId + "','" + unionId + "','" + unitId + "','" + endDate + "')");
