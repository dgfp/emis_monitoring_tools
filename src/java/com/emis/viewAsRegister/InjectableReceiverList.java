 ...
 * @author RHIS082
 */
@WebServlet(name = "InjectableReceiverList", urlPatterns = {"/InjectableReceiverList"})
public class InjectableReceiverList extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        request.setAttribute("submenu", "InjectableReceiverList");
        request.setAttribute("menu", "VIEW AS REGISTER");
        RequestDispatcher view = request.getRequestDispatcher("WEB-INF/jsp/viewAsRegister/InjectableReceiverList.jsp");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (districtId != "" || upazilaId != "" || unionId != "" || unitId != "") {
            try {
                ...
                String sql = "select * from web_injectable_receiver_list(?,?,?,?,?,?,?)";
                System.out.println("SQL: select * from web_injectable_receiver_list(" + divisionId + "," + districtId + ",'" + upazilaId + "','" + unionId + "','" + unitId + "','" + startDate + "','" + endDate + "')");
