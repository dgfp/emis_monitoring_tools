...
//        console.log(geoInformation, additionalInformation, formTag);
    ...
    renderFormElements(formElements){
        ...
        $.each(formElements, function(index, item){
            ...
            formatter = formatter.replace("Element_Label", item["label"]);
            formatter = formatter.replace("Element_Name", item["name"]);
            formatter = formatter.replace("Element_Id", item["id"]);
            $('#globalReportForm').append(formatter);
        });
    },
    getFormElementsFormat(){
        ...
        var element = "<div class='col-md-2 col-xs-4'>\n\
            <select class='form-control select2 select2-hidden-accessible' style='width: 100%;' tabindex='-1' aria-hidden='true' name='Element_Name' id='Element_Id'>\n\
                <option value=''>- Select Element_Label -</option></select></div>";
