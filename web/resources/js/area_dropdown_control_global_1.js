var FormDropDownInit = {
    init: function (geoInformation, additionalInformation, formTag) {
//        console.log(geoInformation, additionalInformation, formTag);
        this.renderFormElements(geoInformation);
        this.renderFormElements(additionalInformation);
    },
    renderFormElements(formElements) {
        ...
        $.each(formElements, function (i, item) {
            if (i == 0 || i % 4 == 0) {
                var appendEl = $('<div class="row"></div>').appendTo('#globalReportForm');
            } else {
                var appendEl = $('#globalReportForm .row:last');
            }
            var formatter = that.getFormElementsFormat();
            formatter = formatter.replaceAll("Element_Label", item["label"]).replace("Element_Name", item["name"]).replace("Element_Id", item["id"]);
            appendEl.append(formatter);
        });
        
        this.loadDivision();
    },
    getFormElementsFormat() {
        var label = "<div class='col-md-1 col-xs-2'><label>Element_Label</label></div>";
        var element = "<div class='col-md-2 col-xs-4'>\n\
            <select class='form-control select2 select2-hidden-accessible' style='width: 100%;' tabindex='-1' aria-hidden='true' name='Element_Name' id='Element_Id'><option value=''>- Select Element_Label -</option></select></div>";
...
//    });
    ...
    $(document).on('change', '#division',function (event) {
        ...
        $("#training-type-container").slideDown("slow");
        $("#dateBlock").slideDown("slow");
        resetAll();
    ...
    $('body #upazila').on('change', '#upazila',function (event) {
        $(".individual").attr('disabled', true);
        resetAll();
    ...
    $(document).on('#union','change',function (event) {
    ...
    $(document).on('#unit','change',function (event) {
    ...
    $(document).on('#village','change',function (event) {
