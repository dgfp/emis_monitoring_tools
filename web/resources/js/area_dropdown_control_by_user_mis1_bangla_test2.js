   ...
   $('#upazila').change(function (event) {
        ...
        if (upazilaId === "" || upazilaId ==='0') {
            ...
            $.get('UnionJsonProvider', {
                upazilaId: upazilaId, zilaId: zilaId, action: "municipality" 
            }, function (response) {
                var returnedData = JSON.parse(response);
...
//END with Manicipility

//Without Manicipility
    ...
    $('#union').change(function (event) {
        ...
        if (unionId === "" || unionId ==='0') {
            ...
            $('<option>').val("").text('- সিলেক্ট করুন -').appendTo(selectTagUnit);
                ...
                $.ajax({
                url: "FWAUnitJsonDataProviderForElcoTest",
                data: {
                    districtId: zilaId,
                    upazilaId: upazilaId,
                    unionId: unionId
