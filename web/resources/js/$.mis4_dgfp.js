            ...
            viewMIS4: function () {
                $(document).off('click', '#showdataButton').on('click', '#showdataButton', function (e) {
                    ...
                    if (data.division === "") {
                        ...
                        //Get Descendant Submission Status
                        $.MIS4.ajax.viewDescendantSubmissionStatus(data);
                    }
                    //$.MIS4.ajax.viewMIS4(data);
                });
            },
...
//                        }
//                    });
                        for (i = 0; i < json.ngo.length; i++) {
                            var row = i + 1;
                            $.each(json.ngo[i], function (k, v) {
                                if (k.match(new RegExp('r_', 'gi')))
                                    k = k.substring(2);
                                $('#form-report-response tbody:last-child tr:nth-child(' + row + ') [name ="' + k + '"]').val(v);
                            });
                        }
            ...
            submitMIS4: function () {
                $('.input-group-approve', '#form-report-response').find('button').click(function (e) {
                    ...
                    if (isValid) {
                        $.ajax({
                            url: "MIS_4_DGFP?action=submitReport&subType=" + json.status,
                            data: data,
                            type: 'POST',
        ...
        ajax: {
            viewMIS4: function (data) {
                ...
                $("#viewStatus").children().fadeOut();
                $.ajax({
                    url: 'MIS_4_DGFP?action=showdata',
                    data: data,
                    type: 'POST',
                    success: function (result) {
                        if (result == "") {
                            toastr["error"]("<b>Data not found</b>");
                        } else {
                            var data = JSON.parse(result);
            ...
            viewDescendantSubmissionStatus: function (data) {
                console.log(data);
                $.ajax({
                    url: 'MIS_4_DGFP?action=descendantSubmissionStatus',
                    data: data,
                    type: 'POST',
                    success: function (result) {
                        var data = [];
                        var facilitySubmissionStatus = [];
                        if (result != null) {
                            data = JSON.parse(result);
                            facilitySubmissionStatus = data["mis4_descendant_submission_status"];
                        }

                        var columns = [
                            {"data": "id", title: 'ক্রমিক নম্বর ',
                                render: function (data, type, row, meta) {
                                    return e2b(meta.row + meta.settings._iDisplayStart + 1);
                            }},
                            {data: "unionname", title: 'ইউনিয়নের নাম', width:'200px'},
                            {data: "mis2", title: 'এম আই এস - ২ ', render:function(data, type, row){
                                    if(data == 1){
                            ...
                            {data: "mis3", title: 'এম আই এস - ৩ ',  render:function(data, type, row){
                                    if(data == 1){
                                        return('<i class="fa fa-check"></i>');
                                    }
                                    else if(data == 0) {
                        ...
                        if ($.fn.DataTable.isDataTable("#descendantSubmissionStatusTable")) {
                            $("#descendantSubmissionStatusTable").DataTable().clear().draw();
                        }
                        $("#descendantSubmissionStatusTable").DataTable(options);
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        toastr["error"]("<b>Request can't be processed</b>");
                    }
                });// End Ajax
//                $("facilitySubmissionStatusTable")
            }
        },
...
//                    }
            ...
            if (json.ngo != undefined) {
                $("#mis4Page19").append("<tr>\n" + getTotalCountRow(new Calc([json.ngo[0]]), 19, json.ngo[0].r_car, "*অসরকারি সংস্থার মোট") + "</tr>");
                $("#mis4Page19").append("<tr>\n" + getTotalCountRow(new Calc([json.ngo[1]]), 19, json.ngo[1].r_car, "*বহুমূখী সংস্থার মোট") + "</tr>");
            }
