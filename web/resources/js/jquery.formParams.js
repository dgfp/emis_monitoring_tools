...
jQuery(function( $ ) {
	var keyBreaker = /[^\[\]]+/g,
		...
		nestData = function( elem, type, data, parts, value, seen ) {
			...
			if ( parts.length ) {
				...
				// case
				if ( name in seen && /checkbox/.test(type) && ! $.isArray( data[ name ] )) {
					if ( name in data ) {
						data[ name ] = [ data[name] ];
					} else {
						data[ name ] = [];
				...
				// Finally, assign data
				if ( /radio|checkbox/.test(type) && ! elem.is(":checked") ) {
					return
				}
	...
	$.fn.extend({
		 ...
		 */
		formParams: function( params, filter ) {
			...
			if ( params ) {
				return this.setParams( params, filter );
			} else {
				return this.getParams( convert, filter );
			}
		},
		setParams: function( params, filter ) {
			...
			this.find(":input[name]").each(function() {
				...
				if ( value !== undefined ) {
					...
					//$this.not('.uncheckable').trigger('change');
				}
			});
		},
		getParams: function( convert, filter ) {
			var data = {},
			...
			this.find(":input[name]")[filter?'filter':'not'](filter).each(function() {
	...
	function flattenData(data){ 
		var	list = {},
			fn = function (data, part) {
				  $.each(data,function(k,v){
					  var p=part?part+'['+k+']':k;
					  ($.type(v)==='object')?fn(v,p):list[p]=v;
				  });
				return list;
			}
		return fn(data);
