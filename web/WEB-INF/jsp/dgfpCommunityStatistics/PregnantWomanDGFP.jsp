<%-- 
    Document   : PregnantWomanDGFP
    Created on : Mar 8, 2021, 3:23:24 PM
    Author     : Helal
--%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@include file="/WEB-INF/jspf/templateHeaderELCO.jspf" %>
<script src="resources/js/area_dropdown_control_by_user_pregnant_dgfp.js"></script>
<script>
    $(function () {
    ...
    $(document).ready(function () {
        ...
        $('#showdataButton').click(function () {
            ...
            if (data.reportType === "individual") {
                ...
                $.ajax({
                    url: "pregnant-woman-dgfp?action=aggregate",
                    data: {
                    ...
                    success: function (json) {
                        ...
                        $("#totalPregnant").html(json.length);
                        $("#activePregnant").html(json.filter(function (d) {
                            return d.active && !d.completed;
                        }).length);
                        $("#duePregnant").html(json.filter(function (d) {
                            return !d.active && !d.completed;
                        }).length);

                        //$("#activePregnant").html("999");
                        //$("#duePregnant").html("999");
                        $("#completedPregnant").html(json.filter(function (d) {
                            return d.completed;
                        }).length);
                        ...
                        reportTitle = "Detail list of registered pregnant women in the selected FWA unit on " + data.endDate;//Set report title
...
//==================================================at a point
                        if (data.viewType == 'atPoint' || data.reportType == "individual") {
                            ...
                            parsedHeader = "<tr>"
                                    ...
                                    + "<th style='vertical-align: top;'>Estimated pregnant woman</th>"
                                    + "<th style='vertical-align: top;'>Registered pregnant women</th>"
                            ...
                            if ($("select#upazila").val() == '0') {
                                ...
                                for (var i = 0; i < json.length; i++) {
                                    var parsedData = "<tr id='centerAlign'><td>" + (i + 1) + "</td>"
                                            ...
                                            + "<td id='centerAlign'>" + finiteFilter(json[i].registered_elco) + "</td>"
                                            + "<td id='centerAlign'>" + (parseInt(json[i].estimated_pregnant_women) || 0) + "</td>"
                                            + "<td id='centerAlign'>" + (parseInt(json[i].registered_pregnant_women) || 0) + "</td>"
                            ...
                            var footerData = "<tr> <td style='text-align:left' colspan='" + colspan + "'>" + getTotalText() + " Total</td>"
                                    ...
                                    + "<td id='centerAlign'>" + Math.round(cal.sum.registered_elco * 0.04) + "</td>"
                                    + "<td id='centerAlign'>" + cal.sum.registered_pregnant_women + "</td>"
                            ...
                            $.each(json, function (index, value) {
                                chartData.push({area: value[area] + " ( n=" + (parseInt(value.estimated_pregnant_women) || 0) + " )", progress: getProgrerss_(value)});
                            });
                            chartData.push({area: getTotalText() + " Total ( n=" + cal.sum.estimated_pregnant_women + " )", progress: getProgrerss_(cal.sum)}); // Total bar
...
//================================periodical
                            ...
                            parsedHeader += "<th style='vertical-align: top;'>Registered pregnant woman (NEW)</th>"
                                    + "</tr>";
                            ...
                            if ($("select#upazila").val() == '0') {
                                ...
                                $.each(json, function (k, v) {
                                    var parsedData = "<tr id='centerAlign'><td>" + (k + 1) + "</td>"
                                            ...
                                            + "<td id='centerAlign'>" + (parseInt(v.registered_pregnant_women) || 0) + "</td>"
                                            + "</tr>";
                                    elco.tableBody.append(parsedData);
                                    elco.tableBodyP.append(parsedData);
                                });
...
//================================Yearly
        ...
        //Functions
        var viewTitle = {
            atPoint: 'Percentage of registered pregnant women on',
            monthly: 'Percentage of registered pregnant women against registered ELCO between',
            yearly: 'Percentage of registered pregnant women between',
            periodical: 'Percentage of pregnant women registered between',
            undefined: 'List of registered pregnant women in the selected FWA unit on' //Individual
        };
//        var viewTitle = {
...
//        };
        ...
        function renderIndividual(elco, json) {
            ...
            for (var i = 0; i < json.length; i++) {
                var status = null;
                if (json[i].completed) {
                    status = "<span class='label label-flat label-info label-xs'>Completed</span>";
                } else {
                    if (!json[i].active) {
                ...
                var parsedData = "<tr><td>" + (i + 1) + "</td>"
                        ...
                        + "<td style='vertical-align: top;'>" + $.app.date(json[i].lmp).date + "</td>"
                        + "<td style='vertical-align: top;'>" + $.app.date(json[i].edd).date + "</td>"
                        + "<td class='center'>" + status + "</td></tr>";
                elco.tableBody.append(parsedData);
                elco.tableBodyP.append(parsedData);
            }
            $("#data-table").DataTable($.app.dtOptions());
...
<section class="content-header"> 
    <h1>Pregnant woman registration status<small></small></h1>
</section>
    ...
    <div class="box box-primary full-screen" id="graphView">
        <div class="box-header with-border" style="padding: 0px;">
            ...
            <div class="box-tools pull-right" style="margin-top: -10px;">
                ...
                <a href="#" id="exportImageChart" download="pregnant_woman_registration_status.png" target="_blank" class="btn btn-flat btn-default btn-xs bold"><i class="fa fa-file-image-o" aria-hidden="true"></i>&nbsp;Image</a>
                <button type="button" class="btn btn-box-tool" id="panel-fullscreen"><i class="fa fa-expand" aria-hidden="true"></i></button>
            </div>
        </div>
...
<script>
    $(document).ready(function () {
        $('#exportPrint').click(function () {
            ...
            frameDoc.document.write('<h3 style="margin-bottom:-12px;text-align:center!important;"><center>Pregnant woman registration status</center></h3>');
        ...
        $('#exportPrintChart').click(function () {
            ...
            //Append the DIV contents.
            frameDoc.document.write('<h3 style="margin-bottom:-12px;text-align:center"><center>Pregnant woman registration status</center></h3>');
            frameDoc.document.write('<h3 style="margin-bottom:-12px;text-align:center!important;"><center>' + $('.viewTitle > .label').html() + '</center></h3>');
        ...
        $("#exportText").click(function (event) {
            var outputFile = "eMIS_pregnant_woman_registration_status";
