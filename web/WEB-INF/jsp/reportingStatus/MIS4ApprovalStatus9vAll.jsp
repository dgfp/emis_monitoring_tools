...
//                $.app.select.$month('select#month').val($.data.month);
            ...
            ajax: {
                statusLoader: function (area) {
                    $('#dashboard').empty();
                    $.ajax({
                        url: "mis4-approval-status-9v-all?action=getStatus",
                        type: "POST",
                        ...
                        success: function (response) {
                            ...
                            if (response.success == true) {
                                ...
                                if ((($.MIS4Status.formData.month == $.MIS4Status.preMonth) && ($.MIS4Status.formData.year == $.MIS4Status.preYear)))
                                    $.MIS4Status.isPreTwoMonth = false;
//                                        NIBRAS AR RAKIB
                                var rows = REPORTING_STATUS.init({text: "upazila", data: response.data, isPreTwoMonth: $.MIS4Status.isPreTwoMonth});
                                $.app.removeWatermark();
                ...
                getProviderList: function (type) {
                    $.ajax({
                        url: "mis4-approval-status-9v-all?action=getProviderList&viewType=" + type,
                        type: "POST",
                        ...
                        success: function (response) {
                            ...
                            if (response.success === true) {
                                ...
                                var columns = [
                                    ...
                                    {data: function (d) {
                                            return $.app.getAssignType(d["assign_type"], 4);
                                        }, title: 'Upazila Type', class: "center type"},
                                    {data: function (d) {
                                            console.log(d);
                                    ...
                                    {data: function (d) {
                                            console.log(d);
                                            var json = JSON.stringify({"zillaid": d.zillaid, "upazilaid": d.upazilaid, "providerid": d.providerid, "provtype": 15, "month": d.month, "year": d.year});
...
//                                        {extend: 'pageLength'}
                ...
                getMIS4: function (json, provider) {
                    $.ajax({
                        url: "mis4-approval-status-9v-all?action=getMIS4",
                        type: "POST",
