...
//=Open Modal For deactive Provider ID====================================================
    ...
    var Catchment = {
        ...
        post: function (data, action) {
            action = action || 'get';
            var options = {url: 'ProviderManagement_RHIS2DB?action=' + action + 'Providerarea', data: data, type: 'POST'};
...
//        };
    var ReportingCatchmentUnion = {
        ...
        generateTable: function () {
            var self = this;
            var columns = [
                {data: "unionnameeng", title: 'Reporting Union Name'},
                {data: function (d) {
                        return $.app.getAssignType(d["assign_type"], 2);
                    }, title: 'Union type'},
                {data: function (d) {
                        return '<a class="btn-reporting-union-unassign btn btn-flat btn-danger btn-xs bold"><i class="fa fa-pencil" aria-hidden="true"></i> Unassigned</a>';
                    }, title: 'Action'}
            ];
            var data = {
        ...
        setup: function () {
            ...
            $.when(UTIL.request("UnionJsonProviderTest", {zilaId: provider["zillaid"], upazilaId: provider["upazilaid"]}, "GET"))
                    .done(function (r1)
                    {
                        UTIL.generateReportingUnion(r1, "", "#reporting_union_id_for_union");
                        $("#geo_assign_type_container").html(UTIL.getAssignTypeDropdown({}));
                        self.generateTable();
                    });
        },
...
//                console.log(e, relatedBtn.data('provider'));
    ...
    var ReportingCatchmentUnit = {
        ...
        generateTable: function () {
            ...
            var columns = [
                {data: "unionnameeng", title: 'Reporting Union Name'},
                {data: "uname", title: 'Reporting Unit'},
                {data: function (d) {
                        return $.app.getAssignType(d["assign_type"], 1);
                    }, title: 'Unit type'},
                {data: function (d) {
                        return '<a class="btn-reporting-unit-unassign btn btn-flat btn-danger btn-xs bold"><i class="fa fa-pencil" aria-hidden="true"></i> Unassigned</a>';
                    }, title: 'Action'}
            ];
            var data = {
                providerid: self.provider["providerid"],
...
//                console.log(e, relatedBtn.data('provider'));
            ...
            $('body').on('change', '#reporting_union_id_for_unit', function (e) {
                var v = $(this).val();
                var payload = JSON.stringify({reporting_unionid: v});
    ...
    var ReportingCatchmentUnassign = function (props) {
        ...
        this.setup = function () {
            console.log(this.allRow, this.currentRow);
            this.$header.css({
                color: '#f58812'
            });
            this.$modal.modal('show');
            this.$header.html(": " + this.provider["provname"]);
        ...
        this.conditionalRendering = function (v) {
...
//            console.log(conditionBlock, conditionBlock.length, v);
        ...
        this.onChangeReasonType = function (v) {
            self.conditionalRendering(v);
        };
        ...
        this.submitUnassign = function (e) {
            ...
            var v = UTIL.validateReportingGeo.validateConditionalRenderingForm(data, formInput);
            if (v) {
        ...
        $('body').on('change', '#unassign_reason_type', {conditionalRendering: this.conditionalRendering}
        , function (event) {
            event.stopImmediatePropagation();
            self.onChangeReasonType($(this).val());
        });
        $('body').on('click', '#submitButtonUnassignCatchment', {form: this.form, inputTags: this.inputTags, provider: this.provider}
        , function (event) {
            event.stopImmediatePropagation();
            self.submitUnassign();
        });
    ...
    $(function () {
        ...
        var ReportingUnitConfig = {
            ...
            tableDataUrl: "report-geo-info?action=getProviderAreaUnit"
        };
...
//===Update provider=========================================================================
        $('#updateProviderModal form').on('submit', function (e) {
            ...
            var url = "ProviderManagement_RHIS2DB?action=updateProvider";
            var url2 = "ProviderManagement_RHIS2DB?action=checkProviderAreaExistance";
                    ...
                    for (var i = 0; i < json.length; i++) {
                        ...
                        if (json[i].provtype == 3) {
                            ...
                            parsedData = parsedData + '<td><button id="btnAssign' + json[i].providerid + '" data-provider=\'' + jsonStr + '\' onclick="assignArea(' + i + ')" class="btn btn-flat btn-info btn-xs bold">' + //data-toggle="modal" data-target="#basicModal"
                                    '<i class="fa fa-map-marker" aria-hidden="true"></i> Assign</button>&nbsp;<button id="btnAssignReportingUnit' + json[i].providerid + '" data-provider=\'' + jsonStr + '\' onclick="assignReportingUnit(' + i + ')" class="btn btn-flat btn-info btn-xs bold btn-assign-reporting-unit hidden">' + //data-toggle="modal" data-target="#basicModal"
                                    '<i class="fa fa-map-marker" aria-hidden="true"></i> Unit</button></td>';
                            ...
                            //var jsonStr = JSON.stringify(json[i]);
                            parsedData += '<td><button id="btnAssignReportingUnion' + json[i].providerid + '" data-provider=\'' + jsonStr + '\' onclick="assignReportingUnion(' + i + ')" class="btn btn-flat btn-info btn-xs bold btn-assign-reporting-union hidden">' + //data-toggle="modal" data-target="#basicModal"
                                    '<i class="fa fa-map-marker" aria-hidden="true"></i> Assign</button></td>';
                            parsedData += '<td>-</td>';
...
<div class="modal fade" id="modalAddProvider" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="post" id="addNewProviderForm" action="ProviderManagement_RHIS2DB?action=addNewProvider" class="form-horizontal" role="form"  autocomplete="off">
...
<div class="modal fade" id="assignReportingUnionModal" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times" aria-hidden="true"></i></button>
                <h4 class="modal-title bold"> Assign Reporting Union</h4>
            </div>
...
<div class="modal fade" id="assignReportingUnitModal" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times" aria-hidden="true"></i></button>
                <h4 class="modal-title bold"> Assign Reporting Unit</h4>
            </div>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="row">
                                <div class="col-md-12 col-xs-12">
                                    <label>Reason <i style="" class="small text-danger">*</i></label>
                                </div>
                                ...
                                <div class="custom-hidden  condition-block" data-show='["1", "2", "3", "4"]'>
                                    <div class="col-md-12 col-xs-12"'>
                                        <label>Activation Date <i style="" class="small text-danger" name="activation_date">*</i></label>
                                    </div>
                                    <div class="col-md-12 col-xs-12">
                                ...
                                <div class="custom-hidden  condition-block" data-show='["1", "2", "3", "4"]'>
                                    <div class="col-md-12 col-xs-12">
                                        <label>HRIS ID <i style="" class="small text-danger">*</i></label>
                                    </div>
                                    <div class="col-md-12 col-xs-12">
                                ...
                                <div id="unassign_district_container" class="custom-hidden condition-block" 
                                     data-show='["1"]'>
                                    <div class="col-md-12 col-xs-12">
                                        <label>Transfered District <i style="" class="small text-danger">*</i></label>
                                    </div>
                                    <div class="col-md-12 col-xs-12">
                                        <div class="all_district_container"></div>
                                    </div>
                                </div>
                            </div>
                            <br/>
