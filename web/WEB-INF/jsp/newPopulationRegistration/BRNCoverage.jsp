<%-- 
    Document   : BRNCoverage
    Created on : Jun 15, 2020, 9:57:06 AM
    Author     : Helal
--%>
...
<section class="content-header">
    <h1>Status of Birth Registration Number Coverage<small></small></h1>
</section>
<!-- Main content -->
...
<script>
    $(function () {
        $.BRN = {
            title: "Status of Birth Registration Number Coverage",
            baseURL: "brn-coverage",
            divid: "#divid",
            zillaid: "#zillaid",
            upazilaid: "#upazilaid",
            unionid: "#unionid",
            ...
            init: function () {
                $.app.removeWatermark();
                $.BRN.events.bindEvent();
            },
            events: {
                bindEvent: function () {
                    $.BRN.events.viewData();
                    //$.BRN.events.populationChartViewType();
                    $.BRN.events.exportPrint();
                    $.BRN.events.exportCSV();
                    $.BRN.events.exportPrintChart();
                },
                viewData: function () {
                    $(document).off('submit', '#showData').on('submit', '#showData', function (e) {
                        ...
                        $.BRN.ajax.viewData(area);
                    });
                },
                exportPrint: function () {
                    $(document).off('click', '#exportPrint').on('click', '#exportPrint', function (e) {
                        $.BRN.dataTableLength = $('select[name=data-table_length]').val();
                        $($.BRN.dataTable).DataTable().page.len(-1).draw();
                        $.export.print($("#export").html(), $.BRN.title + "<br/>" + $('.viewTitle > .label').html(), $.BRN.getArea());
                    });
                },
                exportCSV: function () {
            ...
            ajax: {
                viewData: function (json) {
                    $.ajax({
                        url: $.BRN.baseURL + "?viewType=" + $.BRN.getViewType(json, 0),
                        type: "POST",
                        ...
                        success: function (response) {
                            ...
                            if (response.success == true) {
                                ...
                                var footer = "";
                                $.BRN.chartId.html("");
                                var viewType = $.BRN.getViewType(json, 0);
                                if (viewType == "Unit") {
                                    isUnitWise = true;
                                    klass = "center";
                                }
                                ...
                                if (json.level == "atPoint") {
                                    $('.viewTitle > .label-default').text($.BRN.viewTitle[json.level] + "" + json.endDate);
                                    $.BRN.tableHeader.append($.BRN.getStatusUpToDateHeader(viewType, isUnitWise, klass));
                                    ...
                                    columns = [
                                        ...
                                        {data: function (d) {
                                                ...
                                                return d[$.BRN.getViewType(json, 1)] + "" + unit;
                                            }, class: "viewLevel " + klass},

                                        {data: "eligible"},
                                        {data: function (d) {
                                                return $.app.isFloat(d.havenbrid);
                                            }},
                                        {data: function (d) {
                                                return $.app.isFloat(d.dont_had);
                                            }},
                                        {data: function (d) {
                                                return $.app.isFloat(d.updated_later);
                                            }}
                                    ];
                                    footer = $.BRN.getStatusUpToDateFooter(cal, data.length, $.BRN.getTotalText(json, 0));//Footer
                                    //Chart data preparing
                                    $.each(data, function (i, o) {
                                        $.BRN.chartData.push({
                                            area: o[$.BRN.getViewType(json, 1)] + " ( n=" + o.eligible + " )",
                                            havenbrid: $.app.isFloat(o.havenbrid),
                                            dont_had: $.app.isFloat(o.dont_had),
                                            updated_later: $.app.isFloat(o.updated_later)
                                        });
                                    });
                                    $.BRN.chartData.push({
                                        area: $.BRN.getTotalText(json, 0) + " Total ( n=" + cal.sum.eligible + " )",
                                        havenbrid: ((cal.sum.havenbrid) / data.length).toFixed(1),
                                        dont_had: ((cal.sum.dont_had) / data.length).toFixed(1),
                                        updated_later: ((cal.sum.updated_later) / data.length).toFixed(1)
                                    });
                                    $.chart.renderBar($.BRN.chartId, $.BRN.chartData, $.BRN.chartLabel['atPoint'], $.BRN.chartColor['atPoint'], 100); //chart
                                    


                                } else if (json.level == "monthly") {
                                    $('.viewTitle > .label-default').text($.BRN.viewTitle[json.level] + "" + json.startMonthYear + " and " + json.endMonthYear);
                                    var d = $.BRN.processMonthlyData(data, $.BRN.getViewType(json, 1));
                                    console.log(d);
                                    //Reset data after processing
                                    data = d.tableData;
                                    cal = new Calc(data);
                                    $.BRN.tableHeader.append($.BRN.getMonthlyHeader(viewType, isUnitWise, klass, d.tableHeader));
                                    $("#data-table").DataTable().clear();
                                    ...
                                    footer = $.BRN.getMonthlyFooter(cal, data.length, $.BRN.getTotalText(json, 0), d.months);//Footer
                                    d.chartData.push($.BRN.totalObj);
                                    $.chart.renderBar($.BRN.chartId, d.chartData, d.chartLabel, $.BRN.chartColor['monthly'], 0);
                                }
                                if (viewType == "Unit") {
            ...
            getViewType: function (json, i) {
                if (json.upazilaid == "0")
                    return $.BRN.viewLevel[1][i];
                else if (json.unionid == "0")
                    return $.BRN.viewLevel[2][i];
                else if (json.unitid == "0")
                    return $.BRN.viewLevel[3][i];
                else
                    return $.BRN.viewLevel[4][i];
            },
            getTotalText: function (json, i) {
                if (json.upazilaid == "0")
                    return $.BRN.viewLevel[0][i];
                else if (json.unionid == "0")
                    return $.BRN.viewLevel[1][i];
                else if (json.unitid == "0")
                    return $.BRN.viewLevel[2][i];
            ...
            resetTable: function () {
                $.BRN.tableHeader.empty();
                $.BRN.tableBody.empty();
                $.BRN.tableFooter.empty();
            },
            getStatusUpToDateHeader: function (viewType, isUnitWise, klass) {
                ...
                return '<tr id="tableRow">\
                            ...
                            <th>No. of population registered</th>\
                            <th>Have BRN (%)</th>\
                            <th>Never had BRN (%)</th>\
            ...
            getStatusUpToDateFooter: function (json, length, type) {
                ...
                return '<tfoot id="tableFooter">\n<tr class="bold">\
                                ...
                                <td>' + ((json.sum.havenbrid) / length).toFixed(1) + '</td>\
                                <td>' + ((json.sum.dont_had) / length).toFixed(1) + '</td>\
            ...
            getMonthlyFooter: function (json, length, type, months) {
                $.BRN.totalObj = {};
                $.BRN.totalObj = {area: type + ' Total ( n=' + json.sum.eligible + ' )'};
                var th = "";
                $.each(months, function (i, v) {
                    $.BRN.totalObj["month_" + i] = ((json.sum["month_" + i]) / length).toFixed(1);
            ...
            processMonthlyData: function (data, areaType) {
                ...
                $.each(response.area, function (i, v) {
                    ...
                    $.each(response.months, function (index, value) {
                        ...
                        tableDataObj["month_" + index] = row.have_brid;
                        //Chart data added here
                        chartDataObj["month_" + index] = row.have_brid;
                    });
            ...
            viewTitle: {
                atPoint: "Percent distribution of birth registration number available in registered population up to ",
                monthly: "Percentage of registered population has birth registration number between "
            },
            chartLabel: {
                atPoint: ['Have BRN', 'Never had BRN', 'Could not show (updated later)']
            },
            chartColor: {
