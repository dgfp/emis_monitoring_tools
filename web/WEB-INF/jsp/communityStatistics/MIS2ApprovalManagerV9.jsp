        ...
        Template = $.extend(true, Template, {
            mis: {
                2: {
                    ...
                    9: {viewURL: "MIS2ApprovalProcess?action=showData", mis2ViewURL: "MIS_2?action=showdata"}
                }
            },
            response: null
        });
...
//                    .find('#report').html(data.html);
        ...
        function loadTableData(month, year) {
            Pace.track(function () {
                $.ajax({
                    url: "MIS2ApprovalProcess?action=showData",
                    type: 'POST',
                    data: {
                        month: month,
                        year: year,
        ...
        function loadReviewData(data) {
            Pace.track(function () {
                $.ajax({
                    url: "MIS2ApprovalProcess?action=reviewData",
                    type: 'POST',
                    data: {
...
//            $('#modal-report-response').modal('show', d).find('.overlay')[method]('hidden');
        ...
        function formReportResponse(form, options) {
            var that = form,
                    ...
                    url = "MIS2ApprovalProcess?action=actionOnResponse "; //$(that).attr('action');
