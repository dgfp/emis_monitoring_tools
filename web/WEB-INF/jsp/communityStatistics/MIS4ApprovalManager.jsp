...
<script>
    $(document).ready(function () {
        ...
        Template = $.extend(true, Template, {
            mis: {
                4: {
                    8: {viewURL: "mis4-approval-manager?action=showData", mis4ViewURL: "MIS_4?action=showdata"},
                    9: {viewURL: "mis4-approval-manager?action=showData", mis4ViewURL: "MIS_4?action=showdata"}
                }
            },
            response: null
        });
        ...
        function loadTableData(month, year) {
            Pace.track(function () {
                $.ajax({
                    url: "mis4-approval-manager?action=showData",
                    type: 'POST',
                    data: {
                        month: month,
                        year: year,
        ...
        function loadReviewData(data) {
            Pace.track(function () {
                $.ajax({
                    url: "mis4-approval-manager?action=reviewData",
                    type: 'POST',
                    data: {
        ...
        function formReportResponse(form, options) {
            var that = form,
                    ...
                    url = "mis4-approval-manager?action=actionOnResponse "; //$(that).attr('action');
