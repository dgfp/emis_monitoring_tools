...
<script>
    $(document).ready(function () {
        ...
        Template = $.extend(true, Template, {
            mis: {
                22: {
                    8: {viewURL: "mis2-approval-manager-v9?action=showData", mis2ViewURL: "mis2-9?action=showdata"},
                    9: {viewURL: "MIS2ApprovalProcessDGFP?action=showData", mis2ViewURL: "MIS_2_DGFP?action=showdata"}
                }
            },
            response: null
        });
        ...
        $('<option selected>').val('702').text('MIS 2 (FPI)').appendTo(selectTag);
...
//                }, defaultContent: '-'},
                {data: function (d) {
                        var assign_type = (d.fpi_assign_type === 1) ? "Main" : "Additional";
                        var klass = (d.fpi_assign_type === 1) ? "label-success" : "label-warning";
                        return  "<b>" + d.provname + "</b><br/> <span style='color: #575a5e;'>ID: " + d.providerid 
                                + " Mob: <a href='tel:+880" + d.mobileno + "'>0" 
        ...
        $('#data-table').on('click', 'tbody tr a:not([disabled])', function (e) {
            ...
            if ($(this).is('.action-view')) {
                console.log(data, Template.getData().mis2ViewURL);
                //$('#modal-report-view').modal('show');
                $.ajax({
                    url: Template.getData().mis2ViewURL,
                    data: {
                        divisionId: 93,
                        districtId: data.zillaid,
        ...
        $('#modal-report-view').on('shown.bs.modal', function (e) {
            ...
            $('#reportViewTitle').html(data.modreptitle + " - Provider: " + data.provname + " - ID:" + data.providerid);
            //console.log("Response: "+Template.response.mis2)
            MIS2[Template.version].render(Template.response, Template.response.providers, Template.response.mis2, mis2, Template.response.lmis);
...
//                    .find('#report').html(data.html);
        ...
        function loadTableData(month, year) {
            Pace.track(function () {
                $.ajax({
                    url: "MIS2ApprovalProcessDGFP?action=showData",
                    type: 'POST',
                    data: {
                        month: month,
                        year: year,
        ...
        function loadReviewData(data) {
            Pace.track(function () {
                $.ajax({
                    url: "MIS2ApprovalProcessDGFP?action=reviewData",
                    type: 'POST',
                    data: {
        ...
        function formReportResponse(form, options) {
            var that = form,
                    ...
                    url = "MIS2ApprovalProcessDGFP?action=actionOnResponse "; //$(that).attr('action');
            ...
            data.reporting_unionid = $.data.currentSubmission.reporting_unionid;
            data.unionid = $.data.currentSubmission.unionid;
