<%-- 
    Document   : newjspMIS1ApprovalManagerV9Test
    Created on : Jan 16, 2020, 11:11:42 AM
    Author     : Helal
--%>
        ...
        Template = $.extend(true, Template, {
            mis: {
                1: {
                    ...
                    9: {viewURL: "ReportSubmissionTest?action=showData"}
                }
            },
            getHiddenClass: {
                8: "hidden",
...
//                var klass = (data.fpi_assigntype === 1) ? '':'warning';
//                $(row).addClass(klass);
//            },
            columns: [
                {
                ...
                {data: function (d) {
                        //var assign_type = (d.fpi_assigntype === 1) ? "Main Union" : "Additional Union";
                        //var klass = (d.fpi_assigntype === 1) ? "" : "label-warning";
                        //return d.unionnameeng + "<br />" + "<span class='badge " + klass+ "'>" + assign_type + "</span>" ;
                ...
                {data: function (d) {
                        var assign_type = (d.fwa_assigntype === 1) ? "Main Unit" : "Additional Unit";
                        var klass = (d.fwa_assigntype === 1) ? "label-success" : "label-warning";
                        return  "<b>" + d.provname + "</b><br/> <span style='color: #575a5e;'>ID: " + d.providerid + " Mob: <a href='tel:+880" + d.mobileno + "'>0" + d.mobileno
                                + "</a></span><br/><span class='badge " + klass + "'>" + assign_type + "</span>";
                    }, defaultContent: '-'},
                {data: function (d) {
...
//                console.log("action-response",data.approved);
        ...
        $('#modalReportView').on('shown.bs.modal', function (e) {
            ...
            if (data.data != null) {
                ...
                $.MIS.renderMIS1(data);
                $.app.mis1PageColor(data.fwa_assigntype);
            } else {
        ...
        function loadReviewData(data) {
            Pace.track(function () {
                $.ajax({
                    url: "ReportSubmissionTest?action=reviewData",
                    type: 'POST',
                    data: {
...
//            $('#modal-report-response').modal('show', d).find('.overlay')[method]('hidden');
        ...
        function formReportResponse(form, options) {
            ...
            var currentSubmission = $.data.currentSubmission;
            data.submission_id = firstRow.submission_id;
            data.submission_from = firstRow.submission_from;
            data.submission_to = firstRow.submission_to;
            data.fwaunit = currentSubmission.uname;
            data.month = $('select#month').val();
            data.year = $('select#year').val();
            data.reportType = $('select#reportType').val();

            data.reporting_upazilaid = currentSubmission.reporting_upazilaid;
            data.reporting_unionid = currentSubmission.reporting_unionid;
            data.unionids = currentSubmission.unionids;
            data.unit = currentSubmission.unit;
            data.unitid = currentSubmission.unitid;
            data.fwa_assigntype = currentSubmission.fwa_assigntype;
            console.log('actionOnResponse', data);
...
<%@include file="/WEB-INF/jspf/modal-report-response-v9-dgfp.jspf" %>
