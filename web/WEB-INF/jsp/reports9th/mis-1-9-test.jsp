...
//    };
    ...
    $(document).ready(function () {
        function checkUserDataWithJson(server_json) {
            var district = parseInt($("select#district").val());
            var upazila = parseInt($("select#upazila").val());
            var union = parseInt($("select#union").val());
            var unitid = parseInt($('#unitid').val());
            console.log(district,upazila,union,unitid);
            console.log(server_json["zilla_id"],server_json["reporting_upazilaid"],server_json["bbsunionid"],server_json["unitid"]);
            if ($('#userid').val() == 'fwa_provider' && server_json.isSubmittable) {
                if (district != parseInt(server_json["zilla_id"]) ||
                        upazila != parseInt(server_json["reporting_upazilaid"]) ||
                        union != parseInt(server_json["bbsunionid"]) ||
                        unitid != parseInt(server_json["unitid"])) {
                    return false;
                } else {
                    return true;
                }
...
//            if (!checkUserDataWithJson(json)) {
//                toastr["error"]("<h4><b>পাতা টি রিলোড করুন </b></h4>");
//                return false;
//            }
...
//                $(this).prop("disabled", true);
                //json.LMIS = lmisJon;
                $.ajax({
                    url: "mis-form-1-test?action=submitReport&subType=" + json.status,
                    data: {
                        divisionId: $("select#division").val(),
                        districtId: $("select#district").val(),
                        upazilaId: $("select#upazila").val(),
                        reporting_upazilaid: json.reporting_upazilaid,
                        unionId: $("select#union").val(),
                        bbsunionid: json.bbsunionid,
                        unionids: json.unionids,
...
//                    }
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        var data = JSON.parse(result);
                        console.log(data, $('#isTabAccess').val(), $('#userid').val());

                        console.log(data);
                        var checkData = checkUserDataWithJson(data);
//                        if (!checkData) {
//                            toastr["error"]("<h4><b>পাতা টি রিলোড করুন </b></h4>");
//                            return false;
//                        }
                        ...
                        //For current month additional
                        if (data.serverResponse === "warning") {
                            toastr["error"]("<span class='bold'>অতিরিক্ত ইউনিটের এই মাসের রিপোর্ট জমা দেওয়া হয়নি</span>");
                            $("#assign-add").css("display", "block");
                            $('.mis-form-1').attr('style', 'background: #fce8e5 !important;');
                            $('.table-responsive').attr('style', 'border: 1px solid #fce8e5;');
                            return;
                        }
                        var json = data.MIS, mis = data.MIS, csbaJson = data.CSBA, lmis = data.LMIS, submissionDate = data.submissionDate;

                        //Submit btn access only for FWA and Super admin
                        $('#isSubmitAccess').val() == '99' ? $.RS.submissionButton('show') : $.RS.submissionButton('hide');
                        //For not submitted
                        console.log($('#isSubmitAccess').val(), data);
                        ...
                        if (data.assignType == "1") {
                            $("#assign-main").css("display", "block");
                            $('.mis-form-1').attr('style', 'background: #6AA9F2 !important;');
                            $('.table-responsive').attr('style', 'border: 1px solid #6AA9F2;');
...
//                            $tableUma3.find(_k).html(_v);
    ...
    <section class="content">
        ...
        <input type='hidden' id='unitid' value='${sessionScope.unitid}' />
        <div id="viewStatus">
        </div>
                                ...
                                <div class="insert-lmis-data">
                                    <table class="table-bordered insert-lmis-data-table" style="table-layout: fixed;padding-left: 5px!important">
                                        ...
                                        <tr style="text-align: center;">
                                            <td class="tg-031e" colspan="2" style="text-align: left;">চলতি মাসে কখনও মওজুদ শূন্যতা<br/> হয়ে থাকলে কারণ (কোড) লিখুন <br></td>
                                            <td class="tg-031e ">
